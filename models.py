...
class ChatMessage:
    ...
    # Optional: used when emitting OpenAI-style tool responses
...
def get_clean_message_list(
    ...
    for message in message_list:
        ...
        # Special-case: when the message includes tool_calls (assistant step requesting tools),
        # emit OpenAI-compatible assistant message with tool_calls preserved.
        if message.tool_calls:
            content = message.content
            if content is None:
                content = ""
            # Ensure OpenAI function call arguments are JSON strings
            tool_calls_list = []
                ...
                tool_calls_list.append(
                    {
                        "id": tc.id,
                        "type": tc.type,
                        "function": {"name": tc.function.name, "arguments": fn_args},
                    }
                )
            output_message_list.append({"role": "assistant", "content": content, "tool_calls": tool_calls_list})
            continue

        # Special-case: when the original role was TOOL_RESPONSE and a tool_call_id is present,
        # emit an OpenAI-compatible tool message with tool_call_id, regardless of role conversions.
        # This allows providers that expect tool results as {role: "tool", tool_call_id: ...}.
        if (
            original_role == MessageRole.TOOL_RESPONSE
            and getattr(message, "tool_call_id", None)
            and not flatten_messages_as_text
        ):
            # Build tool message payload
            content = message.content
            # OpenAI expects tool content to be a string
            if isinstance(content, list):
                try:
                    text_parts = [el.get("text", "") for el in content if isinstance(el, dict) and el.get("type") == "text"]
        ...
        if (
            ...
            and isinstance(output_message_list[-1].get("content"), list)
        ):
            ...
            if flatten_messages_as_text:
                ...
                content = message.content
            # Ensure content is never None for providers that require a string
            if content is None:
                content = ""
            output_message_list.append(
                {
