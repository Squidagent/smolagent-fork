...
class ToolCallingAgent(MultiStepAgent):
    ...
    def process_tool_calls(
        ...
        # Store both aggregated observations (for backward compat) and per-call tuples for OpenAI tool responses
        memory_step.observations = memory_step.observations or ""
        memory_step.tool_observations = memory_step.tool_observations or []
        for tool_output in [outputs[k] for k in sorted(outputs.keys())]:
            memory_step.observations += tool_output.observation + "\n"
            memory_step.tool_observations.append((tool_output.id, tool_output.observation))
